...
def run_command(command: str, env: Optional[Dict[str, str]] = None, timeout: int = 30) -> subprocess.CompletedProcess:
    ...
    if env:
        full_env.update(env)
    try:
        return subprocess.run(command, shell=True, capture_output=True, text=True, encoding='utf-8', env=full_env, timeout=timeout)
...
def read_file_lines(file_path: Path) -> List[str]:
    ...
    try:
        ...
        write_log(f"Error reading {file_path}: {str(e)}"); return []
...
def append_to_file(file_path: Path, content: str):
    ...
    try:
        ...
        write_log(f"Error appending to {file_path}: {str(e)}"); raise
...
def save_json_file(file_path: Path, data: Dict):
    ...
    try:
        ...
        write_log(f"Error saving JSON {file_path}: {str(e)}"); raise
